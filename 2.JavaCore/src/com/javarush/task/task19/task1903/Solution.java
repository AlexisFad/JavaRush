    ...
    public static class IncomeDataAdapter implements Customer, Contact{
       ...
       public IncomeDataAdapter(IncomeData data){
        ...
        public String getPhoneNumber() {
            ...
            String phone2 =  Integer.toString(data.getPhoneNumber());
            ArrayList<String> list = new ArrayList<>();

            for (String s: phone2.split("")) list.add(s);
            while (list.size() < 10) list.add(0,"0");

            return (String.format("+%d(%s%s%s)%s%s%s-%s%s-%s%s",data.getCountryPhoneCode(),list.get(0),list.get(1),list.get(2),list.get(3),list.get(4),list.get(5),list.get(6),list.get(7),list.get(8),list.get(9)));
        }
    }
