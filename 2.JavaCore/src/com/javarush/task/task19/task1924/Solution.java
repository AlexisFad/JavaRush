...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        ...
        BufferedReader bufferedReader = new BufferedReader(new FileReader(r.readLine()));

        while (bufferedReader.ready()){
            String string = bufferedReader.readLine();
            String result = "";
            for (String s1: string.split(" ")) {
                if (s1.matches("[0-9]*$") && (Integer.parseInt(s1) >=0 && Integer.parseInt(s1)<=12)) {
                    s1 = map.get(Integer.parseInt(s1));
                }
                result += s1 +" ";
            }
