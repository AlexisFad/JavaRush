...
public class Solution {
    ...
    public static void sort(List<Stock> list) {
        list.sort(new Comparator<Stock>() {
            public int compare(Stock stock1, Stock stock2) {
                ...
                String name = stock1.get("name").toString();
                String name2 = stock2.get("name").toString();
                if (name.compareTo(name2) != 0) return name.compareTo(name2);

                Date date = (Date) stock1.get("date");
                Date date2 = (Date) stock2.get("date");
                if (date.compareTo(date2) != 0) return date.compareTo(date2) == 1? -1 : 1;

                double profit;
                double profit2;
                profit = stock1.containsKey("change") ? Double.parseDouble(stock1.get("change").toString()) :
                        Double.parseDouble(stock1.get("last").toString())
                                - Double.parseDouble(stock1.get("last").toString());

                profit2 = stock2.containsKey("change") ? Double.parseDouble(stock2.get("change").toString()) :
                         Double.parseDouble(stock2.get("last").toString())
                                 - Double.parseDouble(stock2.get("last").toString());

             return Double.compare(profit,profit2) == 1? -1 : 1;
            }
    ...
    public static List<Stock> getStocks() {
        ...
        stocks.add(new Stock("Fake Sirius Satellite", "SIRI",29.56,29.0));
        stocks.add(new Stock("Fake Sirius Satellite", "SIRI",29.50,29.0)); 
        ...
        stocks.add(new Stock("Fake Sirius Satellite", "SIRI", -.04, new GregorianCalendar(2019,10,10).getTime()));
        stocks.add(new Stock("Fake Sirius Satellite", "SIRI", -.03, new GregorianCalendar(2019,10,10).getTime()));
        stocks.add(new Stock("Fake Sirius Satellite", "SIRI", .03, new GregorianCalendar(2019,10,10).getTime()));
